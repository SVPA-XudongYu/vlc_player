...
{
    umask(0);
/* YUX support multiple players
    int pid_file = open("/var/run/lock/vlc.pid", O_CREAT | O_RDWR, 0666);
    int rc = flock(pid_file, LOCK_EX | LOCK_NB);
    printf("vlc lock state is ... pid_file=%d, rc=%d, errno=%d\n", pid_file, rc, errno);
    if(rc != 0 && EWOULDBLOCK == errno) {
        printf("Another vlc is already running...rc=%d\n", rc); // another instance is running
        return -1;
    } else {
        printf("First vlc is running...rc=%d\n", rc);
    }
*/
